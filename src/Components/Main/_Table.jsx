...
export default function SimpleTable() {
  ...
  const [, dispatchDisplayDrawer] = useStateValue();
  const [{ activeRowIndex }, dispatchAtiveRowIndex] = useStateValue();

  const handleOnClick = (event) => {
    dispatchAtiveRowIndex({
  ...
  const handleOnDelete = (e) => {
    e.stopPropagation();
    const filteredItem = workList.filter(
      (item, index) => index !== activeRowIndex
    );
        ...
        <TableBody>
          {workList.map((row, index) => (
            <TableRow
              ...
              style={{
                boxShadow:
                  activeRowIndex === index ? "0 0 10px silver" : "none",
              }}
            >
              <TableCell
                component="th"
